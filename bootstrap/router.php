...
function route_request(): void {
  ...
  if (preg_match('#^/services/([^/]+)/$#', $path, $m)) {
    ...
    // Citeability: canonical must include locale (e.g. /en-us/services/generative-seo/)
    $actualPath = parse_url($_SERVER['REQUEST_URI'] ?? '/', PHP_URL_PATH);
    $canonicalPath = (preg_match('#^/([a-z]{2})-([a-z]{2})/#i', $actualPath)) ? $actualPath : '/en-us' . $path;
  ...
  if (preg_match('#^/insights/([^/]+)/([^/]+)/$#', $path, $m)) {
    ...
    if ($slug === 'content-chunking-seo') {
      ...
      $excerpt = "Google's llms.txt reveals how Google trains LLMs on Search. Turn that blueprint into executable AI SEO strategy, structured data, and technical SEO.";
        ...
        } elseif ($slug === 'agentic-commerce-aps') {
          $articleTitle = "Agentic Commerce: How to Become the Default Vendor for AI Agents";
          $excerpt = "Transition from human-centric SEO to Agentic Commerce. Implement the Agent Procurement Surface (APS) to win automated executions in 2026.";
        } elseif ($slug === 'grounding-queries-fan-out-ai-visibility') {
          $articleTitle = "Grounding Queries vs Fan-Out Queries: The Two Hidden Layers of AI Visibility";
          $excerpt = "AI visibility has two layers: grounding (which sources get cited) and fan-out (which angles get explored). Use Bing AI Performance data to map grounding queries to pages, inject answer blocks, and scale via fan-out ladders. Step-by-step system with templates.";
        } else {
  ...
  // Bay Area hub (region intent — SF, San Jose, Silicon Valley, Peninsula)
  if ($path === '/bay-area/') {
    $actualPath = parse_url($_SERVER['REQUEST_URI'] ?? '/', PHP_URL_PATH);
    if (!preg_match('#^/en-us/bay-area/#', $actualPath)) {
      $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
      $host = $_SERVER['HTTP_HOST'] ?? 'nrlc.ai';
      $queryString = !empty($_SERVER['QUERY_STRING']) ? '?' . $_SERVER['QUERY_STRING'] : '';
      header("Location: " . $scheme . '://' . $host . '/en-us/bay-area/' . $queryString, true, 301);
      return;
    }
    $GLOBALS['__page_meta'] = [
      'title' => 'AI Search Optimization San Francisco Bay Area | Neural Command',
      'description' => 'AI Search Optimization and GEO for the San Francisco Bay Area. San Francisco, San Jose, Palo Alto, Oakland. Book a consultation.',
      'canonicalPath' => '/en-us/bay-area/'
    ];
    render_page('bay-area/index');
    return;
  }

  // Learn Hub Route (Beginner Education)
  if ($path === '/learn/') {
    require_once __DIR__.'/../lib/meta_directive.php';
    $actualPath = parse_url($_SERVER['REQUEST_URI'] ?? '/', PHP_URL_PATH);
    $ctx = [
    ...
    $GLOBALS['__page_meta'] = sudo_meta_directive_ctx($ctx);
    render_page('learn/index');
    return;
  }

  // Learn Pages Routes (Beginner Education)
  if ($path === '/learn/can-ai-do-seo/') {
    require_once __DIR__.'/../lib/meta_directive.php';
    $actualPath = parse_url($_SERVER['REQUEST_URI'] ?? '/', PHP_URL_PATH);
    $ctx = [
      'type' => 'page',
      'slug' => 'learn/can-ai-do-seo',
      'canonicalPath' => $actualPath
    ];
    $GLOBALS['__page_meta'] = sudo_meta_directive_ctx($ctx);
    render_page('learn/can-ai-do-seo');
    return;
  }
  ...
  if (preg_match('#^/tools/([^/]+)/?$#', $path, $m)) {
    ...
    // Bing AI Citations landing page (workflow + CTA)
    if ($toolSlug === 'bing-ai-citations') {
      require_once __DIR__.'/../lib/meta_directive.php';
      require_once __DIR__.'/../lib/helpers.php';
      $locale = function_exists('current_locale') ? current_locale() : 'en-us';
      $canonicalPath = '/' . $locale . $path;
      $ctx = [
        'type' => 'tool',
        'slug' => 'tools/bing-ai-citations',
        'title' => 'Bing AI Citations: How to Use Grounding Queries + Cited Pages to Get Cited More',
        'excerpt' => 'Bing\'s AI Performance report shows which pages get cited and the grounding queries that triggered retrieval. Exact workflow to turn that data into citeable content.',
        'canonicalPath' => $canonicalPath
      ];
      $GLOBALS['__page_meta'] = sudo_meta_directive_ctx($ctx);
      $GLOBALS['__page_meta']['keywords'] = 'Bing AI Performance, grounding queries, cited pages, Bing Webmaster Tools, AI citations, citeable content, Copilot citations, AI visibility';
      render_page('tools/bing-ai-citations');
      return;
    }
