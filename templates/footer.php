...
<!-- Mobile Fixed Action Buttons (Call & Chat) -->
<div class="mobile-fixed-actions" aria-label="Mobile quick actions">
  <!-- Call Button (Left) -->
  <a href="tel:<?= htmlspecialchars(str_replace([' ', '-', '(', ')'], '', gbp_phone())) ?>" class="mobile-fixed-action mobile-fixed-action--call" aria-label="Call <?= htmlspecialchars(gbp_phone()) ?>">
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path>
    </svg>
    <span class="mobile-fixed-action__label">Call</span>
  </a>
  
  <!-- Chat Button (Right) -->
  <button type="button" class="mobile-fixed-action mobile-fixed-action--chat" id="mobile-chat-trigger" aria-label="Open live chat">
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path>
    </svg>
    <span class="mobile-fixed-action__label">Chat</span>
  </button>
</div>

<script>
// Mobile chat button handler
(function() {
  const mobileChatTrigger = document.getElementById('mobile-chat-trigger');
  if (mobileChatTrigger) {
    mobileChatTrigger.addEventListener('click', function() {
      ...
      if (typeof Tawk_API !== 'undefined') {
        ...
        // Tawk.to not loaded yet - wait for it
        const checkTawk = setInterval(() => {
          if (typeof Tawk_API !== 'undefined') {
            clearInterval(checkTawk);
            if (Tawk_API.maximize) {
